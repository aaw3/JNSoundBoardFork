...
namespace JNSoundboard
{
    public partial class MainForm : Form
    {
        ...
        AudioPlaybackEngine playbackEngine1 = new AudioPlaybackEngine(VolumeType.Public);

        //A second output to also output the sound to your headphones or speaker.
        AudioPlaybackEngine playbackEngine2 = new AudioPlaybackEngine(VolumeType.Private);
...
Doesn't affect sounds with custom volumes or that are currently playing.";
        ...
        public MainForm()
        {
            ...
            tooltip.SetToolTip(vsPrivateVolume, SOUND_VOLUME_TOOLTIP);
            tooltip.SetToolTip(nPrivateVolume, SOUND_VOLUME_TOOLTIP);
            
            tooltip.SetToolTip(vsPublicVolume, SOUND_VOLUME_TOOLTIP);
            tooltip.SetToolTip(nPublicVolume, SOUND_VOLUME_TOOLTIP);
            ...
            DisablePublicVolumeChangeEvents();
            DisablePrivateVolumeChangeEvents();
            ...
            privateVolume = XMLSettings.soundboardSettings.PrivateVolume;
            vsPrivateVolume.Volume = privateVolume;
            nPrivateVolume.Value = Helper.linearVolumeToInteger(vsPrivateVolume.Volume);

            publicVolume = XMLSettings.soundboardSettings.PublicVolume;
            vsPublicVolume.Volume = publicVolume;
            nPublicVolume.Value = Helper.linearVolumeToInteger(vsPublicVolume.Volume); //needed because change events are still disabled
            ...
            EnablePublicVolumeChangeEvents();
            EnablePrivateVolumeChangeEvents();
        ...
        private void DisablePrivateVolumeChangeEvents()
        {
            vsPrivateVolume.VolumeChanged -= vsPrivateVolume_VolumeChanged;
            nPrivateVolume.ValueChanged -= nPrivateVolume_ValueChanged;
        }

        private void DisablePublicVolumeChangeEvents()
        {
            vsPublicVolume.VolumeChanged -= vsPublicVolume_VolumeChanged;
            nPublicVolume.ValueChanged -= nPublicVolume_ValueChanged;
        }
        private void EnablePrivateVolumeChangeEvents()
        {
            vsPrivateVolume.VolumeChanged += vsPrivateVolume_VolumeChanged;
            nPrivateVolume.ValueChanged += nPrivateVolume_ValueChanged;
        }

        private void EnablePublicVolumeChangeEvents()
        {
            vsPublicVolume.VolumeChanged += vsPublicVolume_VolumeChanged;
            nPublicVolume.ValueChanged += nPublicVolume_ValueChanged;
        }
        ...
        /*private*/public void stopPlayback()
        {
        ...
        public void playSound(RawSourceWaveStream reader, float Volume, VolumeType type)
        {
            if (Volume == 0)
                return;
            ...
            try
            {
                if (type == VolumeType.Public)
                {
                    playbackEngine1.PlaySound(reader, (publicVolume == 0 ? 0 : Volume));
                }

                if (type == VolumeType.Private)
                {
                    ...
                    {
                        playbackEngine2.PlaySound(reader, (privateVolume == 0 ? 0 : Volume));
                    }
                }
            }
            ...
            catch (NAudio.MmException ex)
            {
                SystemSounds.Beep.Play();
                string msg = ex.ToString();
                MessageBox.Show((msg.Contains("UnspecifiedError calling waveOutOpen") ? "Something is wrong with either the sound you tried to play (try converting it to another format) or your sound card driver\n\n" + msg : msg));
            }
        }

        public string lastPlayedSound;
        private void playSound(string file, float Volume, VolumeType type)
        {
            //stopPlayback();
            ...
            try
            {
                if (type == VolumeType.Public)
                {
                    playbackEngine1.PlaySound(file, Volume);
                }
                ...
                if (type == VolumeType.Private)
                {
                    ...
                    if (SelectedPlaybackDevice2 >= 0 && SelectedPlaybackDevice2 != SelectedPlaybackDevice1)
                    {
                        playbackEngine2.PlaySound(file, Volume);
                    }
                }
            }
            catch (FormatException ex)
            ...
            try
            {
                ...
                if (s != null && s.SoundHotkeys != null && s.SoundHotkeys.Length > 0)
                {
                    ...
                    for (int i = 0; i < s.SoundHotkeys.Length; i++)
                    {
                        ...
                        string soundName = string.IsNullOrWhiteSpace(s.SoundHotkeys[i].SoundName) ? "" : s.SoundHotkeys[i].SoundName;
                        ...
                        //var temp = new ListViewItem(keys); //I changed this
                        var temp = new ListViewItem(soundName);
                        temp.SubItems.Add(keys);
        ...
        public void sortHotkeys()
        {
            ...
            soundHotkeys = soundHotkeys.OrderBy(q => q.SoundName).ToList(); //replaces the method below

            //Removed because it re-sorts the keybinds based on if they have keybind or not (not what I want) also messes with listview because I don't sort it like this
        ...
        private void editSelectedSoundHotkey()
        {
            if (lvKeySounds.SelectedItems.Count > 0)
            {
                ...
                form.editStrings = new string[5];

                form.editStrings[0] = soundHotkeys[selectedIndex].SoundName;
                form.editStrings[1] = Helper.keysArrayToString(soundHotkeys[selectedIndex].Keys);
                form.editStrings[2] = soundHotkeys[selectedIndex].WindowTitle;
                form.editStrings[3] = Helper.fileLocationsArrayToString(soundHotkeys[selectedIndex].SoundLocations);
                //form.editStrings[4] = soundHotkeys[selectedIndex].PublicVolume;
        ...
        {
            //#play selected sound anywhere using lcontrol + lshift + enter (maybe make a toggle feature for this, and a note showing what the keys are)
            if (Keyboard.IsKeyDown(Keyboard.Keys.LControlKey) && Keyboard.IsKeyDown(Keyboard.Keys.LShiftKey) && Keyboard.IsKeyDown(Keyboard.Keys.Enter))
                if (lvKeySounds.SelectedItems.Count > 0) playKeySound(soundHotkeys[lvKeySounds.SelectedIndices[0]]);

            //#stops all sounds using lcontrol + lshift + space
            if (Keyboard.IsKeyDown(Keyboard.Keys.LControlKey) && Keyboard.IsKeyDown(Keyboard.Keys.LShiftKey) && Keyboard.IsKeyDown(Keyboard.Keys.Space))
                stopPlayback();
        ...
        private void playKeySound(XMLSettings.SoundHotkey currentKeysSounds)
        {
            ...
            if (File.Exists(path))
            {
                ...
                {
                    stopPlayback();
                    playSound(path, (privateVolume == 0 ? 0 : customSoundVolume), VolumeType.Private);
                    playSound(path, (publicVolume == 0 ? 0 : customSoundVolume), VolumeType.Public);
                }
                else
                {
                    stopPlayback();
                    playSound(path, (privateVolume == 0 ? 0 : privateVolume), VolumeType.Private);
                    playSound(path, (publicVolume == 0 ? 0 : publicVolume), VolumeType.Public);
                }
        ...
        private bool privateVolumeChangedBySlider = false;
        private bool privateVolumeChangedByField = false;
        private bool publicVolumeChangedBySlider = false;
        private bool publicVolumeChangedByField = false;

        public void vsPrivateVolume_VolumeChanged(object sender, EventArgs e)
        {
            privateVolume = vsPrivateVolume.Volume;

            XMLSettings.soundboardSettings.PrivateVolume = privateVolume;
            saveSettings();

            //prevent infinite or skipped changes
            if (privateVolumeChangedByField)
            {
                privateVolumeChangedByField = false;

                return;
            }

            privateVolumeChangedBySlider = true;

            nPrivateVolume.Value = Helper.linearVolumeToInteger(vsPrivateVolume.Volume);
        }

        public void vsPublicVolume_VolumeChanged(object sender, EventArgs e)
        {
            publicVolume = vsPublicVolume.Volume;

            XMLSettings.soundboardSettings.PublicVolume = publicVolume;
            saveSettings();

            //prevent infinite or skipped changes
            if (publicVolumeChangedByField)
            {
                publicVolumeChangedByField = false;

                return;
            }

            publicVolumeChangedBySlider = true;

            nPublicVolume.Value = Helper.linearVolumeToInteger(vsPublicVolume.Volume);
        }

        public void vsPrivateVolume_MouseWheel(object sender, MouseEventArgs e)
        {
            vsPrivateVolume.Volume = Helper.getNewSoundVolume(vsPrivateVolume.Volume, e.Delta);
        }

        public void vsPublicVolume_MouseWheel(object sender, MouseEventArgs e)
        {
            vsPublicVolume.Volume = Helper.getNewSoundVolume(vsPublicVolume.Volume, e.Delta);
        }

        public void nPrivateVolume_ValueChanged(object sender, EventArgs e)
        {
            //prevent infinite or skipped changes
            if (privateVolumeChangedBySlider)
            {
                privateVolumeChangedBySlider = false;

                return;
            }

            privateVolumeChangedByField = true;

            vsPrivateVolume.Volume = (float)(nPrivateVolume.Value / 100);
        }

        public void nPublicVolume_ValueChanged(object sender, EventArgs e)
        {
            //prevent infinite or skipped changes
            if (publicVolumeChangedBySlider)
            {
                publicVolumeChangedBySlider = false;

                return;
            }

            publicVolumeChangedByField = true;

            vsPublicVolume.Volume = (float)(nPublicVolume.Value / 100);
        }
        ...
        string[] AcceptedFileTypes = new string[] { "mp3", "wav" };
        private void btnAddDirectory_Click(object sender, EventArgs e)
        {
            FolderBrowserDialog dialog = new FolderBrowserDialog();
            dialog.Description = "Select the directory that includes your sounds";
            dialog.RootFolder = Environment.SpecialFolder.MyComputer;
            dialog.ShowNewFolderButton = false;
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                string dir = dialog.SelectedPath;
                var selectedFiles = Directory.EnumerateFiles(dir, "*.*", SearchOption.TopDirectoryOnly).Where(s => AcceptedFileTypes.Contains(Path.GetExtension(s).TrimStart('.').ToLowerInvariant())).ToArray();

                if (MessageBox.Show("Would you like to configure the sounds in the directory individually?", "Individually Configure?", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    for (int i = 0; i < selectedFiles.Length; i++)
                    {
                        ...
                        form.editIndex = -2;
                        form.editStrings = new string[5];
                        form.editStrings[0] = selectedFiles[i].Substring(selectedFiles[i].LastIndexOf('\\') + 1, selectedFiles[i].Length - (selectedFiles[i].LastIndexOf('\\') + 1));
                        form.editStrings[3] = selectedFiles[i];
                        //MessageBox.Show(form.editStrings[0] + " : " + form.editStrings[3]);
                        form.ShowDialog();
                    }
                }
                else
                {
                    for (int i = 0; i < selectedFiles.Length; i++)
                    {
                        string fileName = selectedFiles[i].Substring(selectedFiles[i].LastIndexOf('\\') + 1, selectedFiles[i].Length - (selectedFiles[i].LastIndexOf('\\') + 1));
                        ...
                        newItem.SubItems.Add("");
                        newItem.SubItems.Add("");
                        newItem.SubItems.Add(selectedFiles[i]);

                        newItem.ToolTipText = fileName;

                        lvKeySounds.Items.Add(newItem);

                        soundHotkeys.Add(new XMLSettings.SoundHotkey(fileName, new Keyboard.Keys[] { }, 1, "", new string[] { selectedFiles[i] }));
                    }
        ...
        public bool AudioBoost;
        private void cbAudioBoost_CheckedChanged(object sender, EventArgs e)
        {
            if (cbAudioBoost.Checked)
            {
                if (MessageBox.Show("Are you sure you want to enable the 5x audio boost?\nNote: The boost won't apply while a sound is currently playing.", "Boost Audio?", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                    cbAudioBoost.Checked = false;
            }

            AudioBoost = cbAudioBoost.Checked;
        }

        public bool LoopSound;
        private void cbLoopSound_CheckedChanged(object sender, EventArgs e)
        {
            if (cbLoopSound.Checked)
            {
                if (MessageBox.Show("Are you sure you want to enable the sound looper?\nNote: The sound will not stop looping until the checkbox is unchecked.\nNote: If a new sound is played, the looper will still be active.", "Loop Sounds?", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                    cbLoopSound.Checked = false;
            }
